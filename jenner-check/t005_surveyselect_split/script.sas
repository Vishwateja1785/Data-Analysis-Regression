...
/* Model validation — the repo's 75/25 train/test split via PROC SURVEYSELECT,
   then flagging the selected (training) rows. */
title "Test and Train Sets for Heart Failure Prediction dataset";
proc surveyselect data=heart out=heart_all seed=353678
samprate=0.75 outall; *outall - show all the data selected (1) and not selected (0) for training;
run;
proc print data=heart_all;
run;

/* new_y = HeartDisease for the training set, missing for the test set */
data heart_a;
set heart_all;
